...
'use strict';
import { commands, window, workspace, Disposable, ExtensionContext, Uri, ViewColumn, QuickPickOptions, OutputChannel } from 'vscode';
...
export class CommandCenter extends Disposable {
    ...
    private async restart(currentQuery: string, prompt?: string): Promise<{} | undefined> {
        const query = await window.showInputBox({
            value: currentQuery,
    ...
    private async search(state: State): Promise<{} | undefined> {
        ...
        try {
            ...
            return await this.restart(state.query, `Error parsing glob ${state.query}`);
        }
    ...
    private async select(state: State, items: QuickPickItem[], options?: QuickPickOptions): Promise<{} | undefined> {
