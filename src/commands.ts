...
export class CommandCenter extends Disposable {
	...
	private async files(args: CommandArgs) {
		...
		const limit = this.configuration.get<number>(
			'openFilesConfirmationLimit'
		);
	...
	public toQuickPickItems(uris: Uri[]): QuickPickItem[] {
		return uris
			...
			.filter((_) => _.uri && _.workspace && _.workspace.path)
			.map((mapped) => {
				let item = path.relative(
					mapped.workspace!.path!,
					mapped.uri.fsPath
				);
				const dirName = path.dirname(item);
				let description;
				if (mapped.isMultiWorkspace) {
					description = path.join(
						...[mapped.workspace!.name || '', dirName].filter(
							(_) => _ !== ''
						)
					);
				} else {
				...
				return new QuickPickItem(
					...
					description
					// dirName === '.'
					// 	? mapped.workspace!.name
					// 		? mapped.workspace!.name
					// 		: ''
					// 	: path.join(
					// 			...[
					// 				(mapped.isMultiWorkspace ? mapped.workspace!.name: "") || '',
					// 				dirName,
					// 			].filter((_) => _ !== '')
					// 	  )
				);
